...
class sushiswap {
    async insertToDB(resultAddressOne, resultAddressTwo, pair_dex_id){
        const amounts1 = await sushiRouter.methods.getAmountsOut(1000000, [resultAddressTwo[0].quote_coin_address, resultAddressOne[0].base_coin_address]).call();
        const amounts2 = await sushiRouter.methods.getAmountsOut(1000000, [resultAddressOne[0].base_coin_address, resultAddressTwo[0].quote_coin_address]).call();
        const rates = {
