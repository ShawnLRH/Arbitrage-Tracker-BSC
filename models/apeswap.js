...
class apeswap {
    async insertToDB(resultAddressOne, resultAddressTwo, pair_dex_id){
        const amounts1 = await apeRouter.methods.getAmountsOut(100000, [resultAddressTwo[0].address, resultAddressOne[0].address]).call();
        const amounts2 = await apeRouter.methods.getAmountsOut(100000, [resultAddressOne[0].address, resultAddressTwo[0].address]).call();
        const rates = {
        ...
        console.log(rates.buy + " " + rates.sell);
        connection.query(
