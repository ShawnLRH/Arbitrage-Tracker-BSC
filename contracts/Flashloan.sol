...
contract Flashloan {
  ...
  bytes arbdata;
  enum Direction { ApetoPancake,PancaketoApe  } 
  struct ArbInfo {
        Direction direction;
        uint repayAmount;
          }
  ...
  constructor(address _pancakeFactory, address _apeRouter, address _pancakeRouter02 ) public {
  ...
  function pancakeCall(
    ...
    if(arbInfo.direction == Direction.ApetoPancake) {
          //Buy WBNB on Bakery
          token.approve(address(apeRouter), amountToken); 
          address[] memory path = new address[](2);
          path[0] = address(token0);
          path[1] = address(token1);
          uint[] memory minOuts = apeRouter.getAmountsOut(amountToken, path); 
          apeRouter.swapExactTokensForETH(
        ...
        //Buy WBNB on Bakery
          ...
          path2[0] = address(token1);
          path2[1] = address(token0);
          uint[] memory minOuts2 = apeRouter.getAmountsOut(address(this).balance, path2); 
          apeRouter.swapExactETHForTokens.value(address(this).balance)(
            minOuts2[1], 
            path2, 
            address(this), 
            now
